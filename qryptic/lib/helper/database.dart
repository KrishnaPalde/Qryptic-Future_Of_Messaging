...
Future<Map<String, String>> connectUserViaQR(String qpc) async {
  try {
    ...
    var tokenUrl = Uri.http('15.207.112.204', '/token');
    ...
    // Start QKD session
    var startQKDUrl = Uri.http('15.207.112.204',
    ...
    // Retrieve shared key
    var getKeyURL = Uri.http('15.207.112.204', '/get_shared_key/$qkdSessionId');
  ...
  try {
    ...
    if (tokenResponse.statusCode == 200) {
      ...
      var joinSessionResponse = await http
          .post(Uri.http('15.207.112.204', '/join_qkd/$sessionId'), headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json',
        'Authorization': 'Bearer $token',
      });
